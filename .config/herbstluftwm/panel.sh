...

{
    ...
    while true ; do
        ...
        #CPU
        cpu="^fg()$(echo CPU: `top -b -n1 | grep "Cpu(s)" | awk '{print $2 + $4}'` %)"
        #RAM
        RAM="^fg()$(free -m | awk 'NR==2{printf " RAM: %s/%s MB", $3,$2,$3*100/$2 }')"
         #battery
        ...
        right=" $mpc_current $separator^bg() $cpu $separator^bg() $RAM $separator^bg() $bat $separator^bg() $date $separator"
